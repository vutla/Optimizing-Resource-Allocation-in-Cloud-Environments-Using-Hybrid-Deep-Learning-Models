...
def TADPG(X_train, X_test, y_train, y_test):
    ...
    state_seq = X_train[0:1].astype(np.float32)
    ...
    for t in range(len(X_train)):
        ...
        priority = abs(reward)
        next_state_seq = np.concatenate([state_seq[:, 1:, :], np.expand_dims(np.expand_dims(next_state, axis=0), axis=1)], axis=1)
...
#     X_train = np.random.rand(50, 10, 2).astype(np.float32)
#     X_test = np.random.rand(10, 10, 2).astype(np.float32)
#     y_train = np.random.rand(50)
#     y_test = np.random.rand(10)
#
#     results = TADPG_Optimizer(X_train, X_test, y_train, y_test)
#
