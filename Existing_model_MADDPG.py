...
def MADDPG(X_train, X_test, y_train, y_test):
    ...
    for step in range(len(X_train)):
        t0 = time.time()
        states = [X_train[step, i % X_train.shape[1]] for i in range(num_agents)]
        actions = [agent.act(states[i]) for i, agent in enumerate(agents)]
        next_states = [X_train[(step + 1) % len(X_train), i % X_train.shape[1]] for i in range(num_agents)]
        rewards = [y_train[step % len(y_train)] for _ in range(num_agents)]
